...
import { LaborCostResponse } from "../_services/mutations"
import { useCalculateManualLaborCost } from "../_services/mutations"
...
const formSchema = z.object({
  ...
  cost: z.string({ message: "Maliyet zorunludur" }).min(1, "Maliyet 0'dan büyük olmalı"),
})

type FormValues = z.infer<typeof formSchema>

export default function ManualPriceCalculatorForm() {
  ...
  const mutation = useCalculateManualLaborCost()
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      console.error("Error calculating manual labor cost:", error)
    }
  }
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-between w-full h-full">
        ...
        <Button type="submit" className="w-full h-10 mt-3" disabled={mutation.isPending}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Manuel Hesaplama Sonucu</DialogTitle>
